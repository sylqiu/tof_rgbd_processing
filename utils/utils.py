...
def average_endpoint_error(labels, predictions):
    ...
    with tf.name_scope(None, "average_endpoint_error", (predictions, labels)) as scope:
        ...
        squared_difference = tf.square(predictions - labels)
...
def average_endpoint_error_wconf(labels, predictions, conf):
    """
    Given labels and predictions of size (N, H, W, 2), calculates average endpoint error:
        sqrt[sum_across_channels{(X - Y)^2}]
    """
    ...
    with tf.name_scope(None, "average_endpoint_error", (predictions, labels)) as scope:
        ...
        loss = tf.reduce_sum(squared_difference, 3, keep_dims=True)
        loss = tf.sqrt(loss)
        return tf.reduce_sum(loss * conf) / num_samples

def average_endpoint_error_small(labels, predictions, conf, pw):
    """
    Given labels and predictions of size (N, H, W, 2), calculates average endpoint error:
        sqrt[sum_across_channels{(X - Y)^2}]
    """
    num_samples = predictions.shape.as_list()[0]
    with tf.name_scope(None, "average_endpoint_error_small", (predictions, labels)) as scope:
        predictions = tf.to_float(predictions)
        labels = tf.to_float(labels)
        predictions.get_shape().assert_is_compatible_with(labels.get_shape())

        pw_difference = tf.pow(tf.abs(tf.subtract(predictions, labels)), pw)
        # sum across channels: sum[(X - Y)^2] -> N, H, W, 1
        loss = tf.reduce_sum(pw_difference, 3, keep_dims=True)
        #loss = tf.sqrt(loss)
        return tf.reduce_sum(loss * conf) / num_samples
